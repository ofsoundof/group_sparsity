...
def compute_pindex(modules, percentage, threshold):
    ...
    for m in modules:
        ...
        norm, pindex = get_nonzero_index(weight, dim='output', counter=1, percentage=percentage, threshold=threshold)
...
def modify_submodules(module):
    if isinstance(module, Dense):
        s = module._modules['body']._modules['2'].weight.shape[0]
        module._modules['body']._modules['3'] = nn.Conv2d(s, s, 1, bias=False)
    elif isinstance(module, Transition):
        s = module._modules['2'].weight.shape[0]
        module._modules['4'] = module._modules['3']
        module._modules['3'] = nn.Conv2d(s, s, 1, bias=False)
    else:
...
def set_module_param(module, params):
    # only need to set the parameters of the 3x3 and 1x1 convs
    if isinstance(module, Dense):
        conv1 = module._modules['body']._modules['2']
        conv2 = module._modules['body']._modules['3']
    elif isinstance(module, Transition):
        conv1 = module._modules['2']
        conv2 = module._modules['3']
    else:
        raise NotImplementedError('Do not need to compress the layer ' + module.__class__.__name__)
    conv1.weight.data = params.weight1.data
    conv2.weight.data = params.weight2.data
...
def compress_module_param(module, l, network_pindex):
    ...
    pindex_out = network_pindex[l]
    pl_out = pindex_out.shape[0]
    # print(pl_in, pl_out)
    ...
    # weight1 = torch.index_select(weight1, dim=1, index=pindex_out) # the output channels of conv1 is not compress. Instead compress the output channel of conv2.
    conv1.weight = nn.Parameter(weight1.t().view(ws1[0], pl_in, ws1[2], ws1[3]))
    conv1.in_channels = pl_in

    # compress conv2
    conv2.weight = nn.Parameter(torch.index_select(weight2, dim=1, index=pindex_out).t().view(pl_out, ws2[1], ws2[2], ws2[3]))
    conv2.out_channels = pl_out
...
def modify_network(net_current):
    ...
    for module_cur in modules:
        ...
        if isinstance(module_cur, Dense):
            ...
            raise NotImplementedError('Do not need to compress the layer ' + module_cur.__class__.__name__)

        if args.init_method.find('disturbance') >= 0:
            weight1, weight2 = init_weight_proj(weight1, init_method=args.init_method, d=0, s=0.05)
        else:
            weight1, weight2 = init_weight_proj(weight1, init_method=args.init_method)
...
class Hinge(DenseNet):
    ...
    def calc_sparsity_solution(self, param, reg, sparsity_reg):
        eps = 1e-8
        n = torch.norm(param, p=2, dim=0) # Be careful here. Whether to use dim=0 or dim=1.
        if sparsity_reg == 'l1':
    ...
    def compute_loss(self, batch, current_epoch, converging):
        ...
        for l in range(len(modules)):
            ...
            loss_proj.append(torch.sum(torch.sum(projection.squeeze().t() ** 2, dim=0) ** (q / 2)) ** (1 / q))
            ...
            if not converging and (batch + 1) % 300 == 0:
                ...
                plot_figure(projection.squeeze().t(), l, filename)
        lossp = sum(loss_proj)
    ...
    def calc_regularization(self):
        ...
        for l in range(len(modules)):
            projection = self.sparse_param(modules[l])
            layer_reg.append(torch.norm(projection.squeeze().t(), p=2, dim=0).mean().detach().cpu().numpy())
        return layer_reg
    ...
    def compress_tail(self):
        ...
        # get the compression index
        pindex = torch.cat(self.network_pindex[25:], dim=0)
        pl = pindex.shape[0]
        ...
        # compress the last batchnorm layer
    ...
    def merge_conv(self):
        ...
        for m in self.find_modules():
            if isinstance(m, Dense):
                ...
                raise NotImplementedError('Do not need to deal with the layer ' + m.__class__.__name__)
            conv2 = body._modules['2']
            conv3 = body._modules['3']
            ws = conv2.weight.size()
            ps = conv3.weight.size()
            weight = conv2.weight.data.view(ws[0], -1).t()
            projection = conv3.weight.data.squeeze().t()
            weight = torch.mm(weight, projection).t().view(ps[0], ws[1], ws[2], ws[3])
            if isinstance(m, Dense):
                del body._modules['3']
            elif isinstance(m, Transition):
                body._modules['3'] = body._modules['4']
                del body._modules['4']
            else:
                raise NotImplementedError('Do not need to deal with the layer ' + m.__class__.__name__)
            body._modules['2'].weight = nn.Parameter(weight)
            body._modules['2'].out_channels = ps[0]

    def split_conv(self, state_dict):
        for module in self.find_modules():
            if isinstance(module, Dense):
                s = module._modules['body']._modules['2'].weight.shape[0]
                module._modules['body']._modules['3'] = nn.Conv2d(s, s, 1, bias=False).cuda()
            elif isinstance(module, Transition):
                s = module._modules['2'].weight.shape[0]
                module._modules['4'] = module._modules['3']
                module._modules['3'] = nn.Conv2d(s, s, 1, bias=False).cuda()
            else:
                raise NotImplementedError('Do not need to deal with the layer ' + module.__class__.__name__)
